...
namespace duck_app {
    ...
    public:
        ...
        TexturedEffect(dx_ptr<ID3D11VertexShader>&& vs, dx_ptr<ID3D11PixelShader>&& ps,
                       ...
                       const dx_ptr<ID3D11ShaderResourceView>& texture) : StaticEffect(
            BasicEffect(move(vs), move(ps)), VSConstantBuffers{cbWorld, cbView, cbProj, cbTextureMtx},
        ...
        void SetLightPosition(const ConstantBuffer<DirectX::XMFLOAT4>& buffer) {
            SetVSConstantBuffer(LightPositionSlot, buffer);
        }
    };
}
