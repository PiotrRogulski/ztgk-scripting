...
namespace duck_app {
    ...
    public:
        ...
        MultiTexturedEffect(dx_ptr<ID3D11VertexShader>&& vs, dx_ptr<ID3D11PixelShader>&& ps,
                            ...
                            const ConstantBuffer<DirectX::XMFLOAT4X4>& cbTextureMtx2,
                            const dx_ptr<ID3D11SamplerState>& textureSampler,
                            const dx_ptr<ID3D11ShaderResourceView>& texture0,
                            const dx_ptr<ID3D11ShaderResourceView>& texture1) : StaticEffect(
            BasicEffect(move(vs), move(ps)), VSConstantBuffers{cbWorld, cbView, cbProj, cbTextureMtx1, cbTextureMtx2},
        ...
        void SetTextureMatrixBuffer1(const ConstantBuffer<DirectX::XMFLOAT4X4>& buffer) {
            SetVSConstantBuffer(TextureMtxSlot0, buffer);
        }

        void SetTextureMatrixBuffer2(const ConstantBuffer<DirectX::XMFLOAT4X4>& buffer) {
        ...
        void SetCameraPosition(const ConstantBuffer<DirectX::XMFLOAT4>& buffer) {
            SetVSConstantBuffer(CameraPositionSlot, buffer);
        }

        void SetTexture1(const dx_ptr<ID3D11ShaderResourceView>& texture) {
            SetPSShaderResource(TextureSlot0, texture);
        }

        void SetTexture2(const dx_ptr<ID3D11ShaderResourceView>& texture) {
            SetPSShaderResource(TextureSlot1, texture);
        }

        void SetTextures(const dx_ptr<ID3D11ShaderResourceView>& texture0,
